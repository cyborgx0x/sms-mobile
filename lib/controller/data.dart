...
importSample() async {
  ...
  SMSSample.forEach((element) { smsprovider.insert(SMSItem.fromMap(element));});
  var data = smsprovider.getAll();
  data.then((value) => value.forEach((element) {smsprovider.delete(element["id"]);}));
}
...
Future<Map<String, SMSItem>> getFilterConversationListDB(filter) {
  Future<Map<String, SMSItem>> allSMS;
  allSMS = controllerGetAll().then((value) => filterConversation(filter, value));
  return allSMS;
}
...
List<SMSItem> processSMSList(listSMS) {
  ...
  listSMS.forEach((element) {listSMSbyPhone.add(SMSItem.fromMap(element));});
